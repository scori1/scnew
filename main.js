...
kunz.browserDescription = ["GiiSkyze", "Browser", "3.0.0"];
kunz.on('qr', () => {
console.log(color('[ Gii ]', 'red'), color('Scan me'))
})
fs.existsSync(`./session.json`) && kunz.loadAuthInfo(`./session.json`)
kunz.on('connecting', () => {
console.log(color('[ Gii ]', 'red'), color('Wait!'))
})
kunz.on('open', () => {
console.log(color('[ Gii ]', 'red'), color('Connected!'))
})
await kunz.connect({
timeoutMs: 30 * 1000
})
